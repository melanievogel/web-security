...
export class HeroService {
  ...
  getBeverages() {
    return this.http.get<Hero[]>(AppSettings.API_ENDPOINT + '/shopping')
      .pipe(map(data => data), catchError(this.handleError))
  }

  getBeverageDetail(id: number): Observable<Hero>{
    return this.getBeverages().pipe(
      map(beverages => beverages.find(beverage => beverage.id === id))
    )
  }
